...
internal partial class FAStringDfaTableRunner : FAStringRunner
{
	...
	private FAMatch _NextImpl(string str)
	{
		int tlen;
		int tto;
		int prlen;
		...
		int ch = -1;
		Advance(str, ref ch, ref len, true);
		start_dfa:
		acc = _dfa[state];
		++state;
		tlen = _dfa[state];
		...
		for (i = 0; i < tlen; ++i)
		{
			...
			for (j = 0; j < prlen; ++j)
			{
				...
				{
					Advance(str, ref ch, ref len, false);
					state = tto;
					goto start_dfa;
				}
			}
		...
		if (acc != -1)
		{
			...
			if (be != null)
			{
				...
				if (acc != -1)
				{
					return FAMatch.Create(sym, str.Substring((int)cursor_pos, len), cursor_pos, line, column);
				}
				if (ch == -1)
				{
					return FAMatch.Create(-1, str.Substring((int)cursor_pos, len), cursor_pos, line, column);
				}
				Advance(str, ref ch, ref len, false);
				state = 0;
				goto start_be_dfa;
			}
			return FAMatch.Create(acc, str.Substring((int)cursor_pos, len), cursor_pos, line, column);
		}
